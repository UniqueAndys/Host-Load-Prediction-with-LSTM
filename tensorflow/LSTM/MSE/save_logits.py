...
flags.DEFINE_string("model", "gru", "The RNN type")
...
def run_train_epoch(session, m, data_x, data_y, eval_op):
    cost_sum = []
    state_sum = []
    for i in range(int(len(data_y) / FLAGS.batch_size)):
        ...
        state_sum.append(state)
    return (np.mean(cost_sum), state_sum)
    
def run_test_epoch(session, m, data_x, data_y, eval_op, train_state):
    cost_sum = []
    logits_sum = []
    for i in range(int(len(data_y) / FLAGS.batch_size)):
        cost, logits, _ = session.run(
            [m.cost, m.logits, eval_op],
            {m.input_data: data_x[i*FLAGS.batch_size:(i+1)*FLAGS.batch_size],
             m.targets: data_y[i*FLAGS.batch_size:(i+1)*FLAGS.batch_size],
             m.initial_state: train_state[i]})
        cost_sum.append(cost)
        logits_sum.append(logits)
    return (np.mean(cost_sum), logits_sum)    

def main(_):
    print("===============================================================================")
    print("The input_dim is", FLAGS.input_dim, "The hidden_dim is", FLAGS.hidden_dim, "The output_dim is", FLAGS.output_dim)
    ...
    print("The path is ", FLAGS.data_path)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        for i in range(FLAGS.epoch):
            ...
            test_loss, logits = run_test_epoch(session, m_test, X_test, y_test, tf.no_op(), train_state)
            if i == 0:
                train_best = train_loss
                test_best = test_loss
                test_cost_best = np.asarray(logits)
            ...
            if i == FLAGS.epoch - 1:
                ...
                print(test_cost_best.shape)
                save_path = "./logits/"+str(FLAGS.model)+"_"+str(FLAGS.output_dim)+".pkl"
                with open(save_path,'wb') as output:
                    pickle.dump(test_cost_best, output)
